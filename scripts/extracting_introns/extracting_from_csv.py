...
parser.add_argument("-i", "--csv_input", type=str, required=True)
...
with open(f'{filename}_intron.fa', 'w') as intron_file, open(args.csv_input, 'r') as inp:
    for line in inp:
        seq_id, Start, End, strand, uniquely_mapped, multi_mapped, mapped_sum = line.split(
            ',')
        mapped_sum = mapped_sum.replace('\n', '')
        if seq_id != "contig":
            Start = int(Start)
            End = int(End)
            End = End + 1
            seq_full = fasta_index[seq_id].seq
            id_full = f'>{seq_id}.{Start}-{End}-{strand}'
            ...
            if trimming_type == "splice2deep":
                with open(f'{filename_donor}_intron.fa', 'a') as donor_intron_file, open(f'{filename_acceptor}_intron.fa', 'a') as acceptor_intron_file:
                    ...
                    acceptor_start = End - flanking_size

                    # end = End + 300
                    donor_end = Start + flanking_size
                    ...
                    if donor_start <= 0 or acceptor_start <= 0:
                        ...
                        if acceptor_end >= contig_lengths[seq_id] or donor_end >= contig_lengths[seq_id]:
                            print(
                                f'Short end border on {id_full} {Start} {End} {contig_lengths[seq_id]}')
                        else:
                            ...
                            if strand == "+":
                                ...
                                donor_seq = seq_full[donor_start:(donor_end+2)].upper(
                                )
                                acceptor_seq = seq_full[acceptor_start-2:(acceptor_end)].upper(
                                )
                            elif strand == "-":
                                acceptor_seq = seq_full[(donor_start-2):(donor_end)].upper(
                                )
                                donor_seq = seq_full[(acceptor_start-2):(acceptor_end)].upper(
                                )
                                ...
                                print(
                                    f'Strand {strand} is not + or - (seq_id {seq_id}')
                ...
                if start <= 0:
                    print("start ", id_full, start)
                    start = 1

                if end >= contig_lengths[seq_id]:
                    print("end ", id_full, end, contig_lengths[seq_id])
                    end = contig_lengths[seq_id]
