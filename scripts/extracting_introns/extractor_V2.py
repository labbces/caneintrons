...
parser.add_argument("-t", "--trimming_type", choices=['IMEter', 'spliceator', 
                    'splice2deep'], help='', default='IMEter')
...
filename_donor = filename + "donor"
filename_acceptor = filename + "acceptor"
            ...
            with open(f'{filename_donor}_intron.fa', 'w') as donor_intron_file:
                with open(f'{filename_acceptor}_intron.fa', 'w') as acceptor_intron_file:
                    ...
                    if donor_start <= 0 or acceptor_start <= 0:
                        # start = 1
                        print(f'Short start border on {id_full} {Start} {End} {contig_lengths[seq_id]}')
                    else: 
